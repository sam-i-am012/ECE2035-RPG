...
{
    GameInputs in; // struct that will be returned that has all inputs recorded
    ...
    //button.read() returns 1 if pressed and 0 if released
    //invering the readings so it makes more sense from a physical standpoint
    //this will now become:
    //    pressed: 0 (inactive, closed circuit)
    //    not pressed: 1 (active, open circuit)
    in.b1 = !button1.read(); 
    wait(0.03); //might help prevent user from spamming the button ? i think ? 
    in.b2 = !button2.read();
    wait(0.03);
    in.b3 = !button3.read();
